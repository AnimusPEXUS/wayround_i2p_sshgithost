...
class SFTPServerInterface(paramiko.sftp_si.SFTPServerInterface):
    ...
    def translate_path(self, path):
        ...
        ap = wayround_i2p.utils.path.abspath(
    ...
    def canonicalize(self, path):
        return wayround_i2p.utils.path.normpath(path)
    ...
    def list_folder(self, path):
        ...
        path = wayround_i2p.utils.path.realpath(path)
    ...
    def lstat(self, path):
        ...
        path_paths = wayround_i2p.utils.path.file_paths(path)
    ...
    def rename(self, oldpath, newpath):
        ...
        oldpath_paths = wayround_i2p.utils.path.file_paths(oldpath)
        newpath_paths = wayround_i2p.utils.path.file_paths(newpath)
...
def channel_exec_cmd(channel, cmd):
    ...
    t1 = wayround_i2p.utils.stream.cat(
        p.stdout,
        channel,
        threaded=True,
    ...
    t2 = wayround_i2p.utils.stream.cat(
        channel,
        p.stdin,
        threaded=True,
    ...
    t3 = wayround_i2p.utils.stream.cat(
        p.stderr,
        channel,
        threaded=True,
        read_method_name='read',
...
class ServerInterface(paramiko.server.ServerInterface):
    ...
    def check_channel_exec_request(self, channel, command):
        ...
        if prog in [
            ...
            np = wayround_i2p.utils.path.join(
                self._ssh_git_host.get_working_root_dir(),
                parsed_cmd[1]
                )
...
class SSHGitHost:
    ...
    def __init__(
        ...
        self._working_root_dir = \
            wayround_i2p.utils.path.realpath(working_root_dir)
    ...
    def check_is_path_outside(self, path):
        return not wayround_i2p.utils.path.is_subpath(
            path,
            self.get_working_root_dir()
            )

    def check_is_path_outside_real(self, path):
        return not wayround_i2p.utils.path.is_subpath_real(
            path,
            self.get_working_root_dir()
            )
    ...
    def home_validate_name(self, name):
        ...
        try:
            jid = wayround_i2p.xmpp.core.jid_to_bare(name)
        except:
            raise
...
def get_levels(root_path, path):
    ...
    sub_path = wayround_i2p.utils.path.split(
        wayround_i2p.utils.path.get_subpath(
            root_path,
            path
            )
        )
